...
| MERGE NOTE
|------------------------------------------------------------------------
| UI/JSX in this file is kept EXACTLY as the original ("old") attempt
| page — no visual/layout/className changes.
|
| Timer logic (pause/resume, heartbeat sync, live vs self-paced mode
| handling, sendBeacon on unload) has been merged in from the newer
...
function createQuestionState(questions, savedAnswers) {
  const savedMap = new Map(
    (Array.isArray(savedAnswers) ? savedAnswers : []).map((item) => [
    ...
    const selectedIndex =
      saved?.selectedOptionId != null
        ? question.options.findIndex(
            (option) => Number(option.id) === Number(saved.selectedOptionId)
          )
        : null;

    acc[Number(question.id)] = {
      visited: saved ? Boolean(saved.visited) : index === 0,
      selectedOption: selectedIndex >= 0 ? selectedIndex : null,
      markedForReview: saved ? Boolean(saved.markedForReview) : false,
...
function getStatus(item) {
  if (!item?.visited) return STATUS.NOT_VISITED;
  if (item.markedForReview && item.selectedOption !== null)
    return STATUS.ANSWERED_REVIEW;
  if (item.markedForReview) return STATUS.REVIEW;
  if (item.selectedOption !== null) return STATUS.ANSWERED;
  return STATUS.NOT_ANSWERED;
}

function PaletteIcon({ status, number, current }) {
  const base =
...
export default function AttemptPage() {
  ...
  const attemptId = Number(searchParams.get("attemptId") || 0);
  ...
  const [desktopPalette, setDesktopPalette] = useState(true);
  ...
  const forceSubmitTimerRef = useRef(null);
  const submitTestRef = useRef(null);
  const enterFullscreenRef = useRef(null);

  useEffect(() => {
    stateRef.current = questionsState;
  }, [questionsState]);
  ...
  const timedSections = useMemo(
    () =>
      sections.filter((section) => Number(section.durationMinutes || 0) > 0),
    [sections]
  );

  const sectional = Boolean(test?.sectional) && timedSections.length > 0;

  const fixedTimer = !sectional && Number(test?.durationMinutes || 0) > 0;

  const isDpp =
    String(test?.categorySlug || test?.category || "")
      .trim()
      .toLowerCase() === "dpp" || Boolean(test?.isDpp);
  ...
  const activeSection =
    timedSections[activeSectionIndex] || timedSections[0] || null;

  const sectionQuestions = useMemo(() => {
    if (!sectional || !activeSection) {
      return questions;
    }

    const direct = questions.filter(
      (question) => Number(question.sectionId) === Number(activeSection.id)
    );
    ...
    return questions;
  }, [questions, sectional, activeSection]);

  const currentSectionQuestionIndex = Math.max(
    0,
  ...
  */
  const isLastQuestionOfCurrentSection =
    ...
    Number(sectionQuestions[sectionQuestions.length - 1]?.id) ===
  ...
  */
  const isLastQuestionOfEntireTest =
    Boolean(currentQuestion) &&
    questions.length > 0 &&
    Number(questions[questions.length - 1]?.id) === Number(currentQuestionId);
  ...
  */
  const currentSectionHasNextSection =
    sectional && activeSectionIndex < timedSections.length - 1;

  const sectionBoundaryLocked =
    isLastQuestionOfCurrentSection && currentSectionHasNextSection;

  const allowedQuestionIds = useMemo(
    () => new Set(sectionQuestions.map((question) => Number(question.id))),
    [sectionQuestions]
  );
  ...
  const answeredCount = useMemo(
    () =>
      questions.filter(
        (question) => questionsState[question.id]?.selectedOption !== null
      ).length,
  ...
  const markedCount = useMemo(
    () =>
      questions.filter(
        (question) => questionsState[question.id]?.markedForReview
      ).length,
    [questions, questionsState]
  );
  ...
  const answeredReviewCount = useMemo(
    () =>
      questions.filter((question) => {
        const state = questionsState[question.id];
        return state?.markedForReview && state?.selectedOption !== null;
      }).length,
    [questions, questionsState]
  );

  const reviewOnlyCount = Math.max(0, markedCount - answeredReviewCount);

  const visitedCount = useMemo(
    () =>
      questions.filter((question) => questionsState[question.id]?.visited)
        .length,
    [questions, questionsState]
  );
  ...
  const notAnsweredCount = Math.max(
    0,
    questions.length - answeredCount - notVisitedCount
  );
  ...
  */
  const goToQuestion = useCallback(
    (questionId) => {
       ...
       */
      if (sectional && !allowedQuestionIds.has(Number(question.id))) {
        return;
      }
    ...
    [questions, sectional, allowedQuestionIds]
  );
  ...
  */
  const getChangedAnswers = useCallback(() => {
    ...
    for (const question of questions) {
      const item = current[question.id];
      ...
      const selectedOptionId =
        item.selectedOption == null
          ? null
          : Number(question.options?.[item.selectedOption]?.id);
      ...
      const previous = snapshot[question.id];

      if (
        !previous ||
        previous.selectedOptionId !== payload.selectedOptionId ||
        previous.visited !== payload.visited ||
        previous.markedForReview !== payload.markedForReview ||
        previous.timeSpentSeconds !== payload.timeSpentSeconds
      ) {
  ...
  const saveCheckpoint = useCallback(
    async ({ force = false, keepalive = false } = {}) => {
      ...
      const changed = getChangedAnswers();

      if (changed.length === 0 && !force) {
        return true;
      }

      if (checkpointBusy && !keepalive) {
        return false;
      }
      ...
      try {
        const response = await fetch(
          `/api/test/${encodeURIComponent(String(series))}/${encodeURIComponent(
            String(id)
          )}/checkpoint`,
          {
            method: "POST",
            ...
            credentials: "include",
            body: JSON.stringify({
              attemptId: Number(attemptRef.current.id),
              answers: changed,
            }),
        ...
        for (const item of changed) {
          snapshotRef.current[item.questionId] = item;
        }
    ...
    [getChangedAnswers, checkpointBusy, series, id, router, questions.length]
  );
  ...
  */
  const sendTimerAction = useCallback(
    async (action, { keepalive = false } = {}) => {
      const currentAttemptId = Number(attemptRef.current?.id || 0);

      if (!currentAttemptId || submittedRef.current) {
        return null;
      }

      if (timerActionBusyRef.current && action !== "pause") {
        return null;
      }
      ...
      try {
        ...
        const mode = String(data?.timerMode || "").toLowerCase();

        if (mode === "active") {
          const activeSeconds = Math.max(0, Number(data?.activeSeconds || 0));
          const remaining = Math.max(0, Number(data?.remainingSeconds || 0));
          ...
          if (currentAttemptId === Number(attemptRef.current?.id)) {
            attemptRef.current = {
              ...attemptRef.current,
              activeSeconds,
            ...
            setAttempt((prev) =>
              prev?.id === currentAttemptId
                ? { ...prev, activeSeconds, remainingSeconds: remaining }
                : prev
            );
          }
        }

        timerRunningRef.current = action !== "pause";
        ...
        timerActionBusyRef.current = false;
      }
    },
    [id, isDpp, router, series]
  );
  ...
  */
  const submitTest = useCallback(
    async (auto = false) => {
      if (submittedRef.current || submitting || !attemptRef.current?.id) {
        return;
      }
      ...
      try {
        const timerMode = String(
          attemptRef.current?.timerMode ||
            (attemptRef.current?.eventId ? "live" : "active")
        ).toLowerCase();

        if (timerMode === "active") {
          await sendTimerAction("heartbeat");
        }

        const checkpointSaved = await saveCheckpoint({ force: true });
        ...
        const response = await fetch(
          ...
          )}/submit`,
          {
            ...
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({ attemptId: Number(attemptRef.current.id) }),
          }
        );
        ...
        try {
          ...
          localStorage.removeItem(`spiderman_attempt_${data.result.attemptId}`);
        } catch {}
        ...
        router.replace(
          `/test/${encodeURIComponent(String(series))}/${encodeURIComponent(
            String(id)
          )}/result?attemptId=${encodeURIComponent(data.result.attemptId)}`
        );
      } catch (error) {
        console.error("Submit error:", error);
        toast.error(error?.message || "Unable to submit test.");
      } finally {
        setSubmitting(false);
      }
    },
    [id, series, router, saveCheckpoint, sendTimerAction, submitting]
  );
    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(
          `/api/test/${encodeURIComponent(String(series))}/${encodeURIComponent(
            String(id)
          )}/attempt?attemptId=${encodeURIComponent(String(attemptId))}`,
        ...
        if (!response.ok) {
          ...
          throw new Error(data?.error || "Unable to load test.");
        }
        ...
        const loadedAttempt = data?.attempt || null;
        ...
        if (!loadedQuestions.length) {
          throw new Error("No questions were found for this test.");
        }

        const normalizedSections = loadedSections.map((section) => ({
          ...section,
          id: Number(section.id ?? section.sectionId),
          sectionName:
            section.sectionName ?? section.section_name ?? "Section",
          durationMinutes: Number(
            section.durationMinutes ?? section.duration_minutes ?? 0
          ),
          questionCount: Number(
            section.questionCount ?? section.question_count ?? 0
          ),
          isSequential: Boolean(
            section.isSequential ?? section.is_sequential ?? false
          ),
          timerGroup: section.timerGroup ?? section.timer_group ?? null,
        }));

        const normalizedQuestions = loadedQuestions.map((question) => ({
          ...question,
          id: Number(question.id ?? question.questionId),
          sectionId:
            question.sectionId != null
              ? Number(question.sectionId)
              : question.section_id != null
              ? Number(question.section_id)
              : null,
           ...
           */
          questionImageUrl:
            question.questionImageUrl ?? question.question_image_url ?? null,

          marks: Number(question.marks ?? 0),
          negativeMarks: Number(
            question.negativeMarks ?? question.negative_marks ?? 0
          ),
          options: (
            ...
            id: Number(option.id),
            label:
              option.label ?? option.optionLabel ?? option.option_label ?? "",
            text: option.text ?? option.optionText ?? option.option_text ?? "",
          })),
        }));

        const restored = createQuestionState(
          normalizedQuestions,
          data?.savedAnswers ?? data?.saved_answers ?? []
        );
        ...
        try {
          ...
          if (backup) {
            ...
            if (
              ...
              typeof parsed.questionsState === "object"
            ) {
              Object.entries(parsed.questionsState).forEach(
                ([questionId, item]) => {
                  restored[questionId] = {
                    ...(restored[questionId] || {}),
                    ...item,
        ...
        setQuestionsState(restored);

        const firstQuestion =
          normalizedQuestions.find(
            (question) => restored[question.id]?.visited
          ) || normalizedQuestions[0];

        setCurrentQuestionId(firstQuestion ? Number(firstQuestion.id) : null);

        /*
         * Restore timer state from server.
         *
         * LIVE:
         * wall-clock deadline remains authoritative.
         *
         * ACTIVE:
         * activeSeconds is the persisted usage and
         * lastActiveAt determines whether the current
         * interval is running or paused.
         */
        const loadedTimerMode = String(
          loadedAttempt?.timerMode ||
            (loadedAttempt?.eventId ? "live" : "active")
        ).toLowerCase();

        const restoredActiveSeconds = Math.max(
          0,
          Number(
            loadedAttempt?.activeSeconds ?? loadedAttempt?.active_seconds ?? 0
          ) || 0
        );
        ...
        const restoredRemainingSeconds = Math.max(
          0,
          Number(
            loadedAttempt?.remainingSeconds ??
              loadedAttempt?.remaining_seconds ??
              0
          ) || 0
        );

        /*
         * A self-paced attempt whose lastActiveAt is
         * NULL is paused.
         *
         * A live attempt is always running because its
         * wall-clock deadline continues independently.
         */
        if (loadedTimerMode === "live") {
          timerRunningRef.current = true;

          const liveStartedAt = getTimestamp(loadedAttempt?.startedAt);

          if (Number.isFinite(liveStartedAt)) {
            setElapsedSeconds(
              Math.max(0, Math.floor((Date.now() - liveStartedAt) / 1000))
            );
          }
        } else {
          timerRunningRef.current = Boolean(
            loadedAttempt?.lastActiveAt || loadedAttempt?.last_active_at
          );

          setElapsedSeconds(restoredActiveSeconds);
          ...
          if (!isDpp) {
            setRemainingSeconds(
              restoredRemainingSeconds ||
                Math.max(
                  0,
                  Number(loadedTest?.durationMinutes || 0) * 60 -
                    restoredActiveSeconds
                )
            );
          }
        }
        ...
        if (!cancelled) {
          setLoadError(error?.message || "Unable to load test.");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  */
  useEffect(() => {
    if (!attempt?.id || !questions.length) {
      return;
    }
    ...
    saveTimerRef.current = window.setTimeout(() => {
      try {
        localStorage.setItem(
          ...
          JSON.stringify({
            questionsState: stateRef.current,
  ...
  }, [questionsState, attempt?.id, questions.length]);
  ...
  */
  useEffect(() => {
    if (!attempt?.id || loading || submittedRef.current) {
      return;
    }
    ...
    const timer = window.setInterval(() => {
      ...
      void sendTimerAction("heartbeat");
      saveCheckpoint();
    }, 60000);

    return () => window.clearInterval(timer);
  }, [attempt?.id, loading, saveCheckpoint, sendTimerAction]);
  ...
  */
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      if (!currentAttempt?.id || submittedRef.current) {
        return;
      }

      const timerMode = String(
        currentAttempt?.timerMode ||
          (currentAttempt?.eventId ? "live" : "active")
      ).toLowerCase();
      ...
      if (timerMode === "live") {
        ...
        const deadlineAt = getTimestamp(
          currentAttempt?.deadlineAt || currentAttempt?.deadline_at
        );

        const startedAt = getTimestamp(currentAttempt?.startedAt);
       ...
       * -------------------------------------------------------------
       *
       * Only count active time while page is visible/running.
       */
      if (!timerRunningRef.current || document.visibilityState !== "visible") {
        return;
      }
      ...
      const activeSeconds = Math.max(
        0,
        Number(
          currentAttempt?.activeSeconds ?? currentAttempt?.active_seconds ?? 0
        ) || 0
      );

      const nextActiveSeconds = activeSeconds + 1;

      currentAttempt.activeSeconds = nextActiveSeconds;

      setElapsedSeconds(nextActiveSeconds);

      /*
       * DPP:
       * active-time stopwatch.
       */
      if (isDpp) {
        return;
      }
      ...
      /*
       * -------------------------------------------------------------
       * SECTIONAL SELF-PACED TIMER
       * -------------------------------------------------------------
       */
      if (sectional && timedSections.length) {
        let cursor = nextActiveSeconds;
        let activeIndex = timedSections.length - 1;
        let remaining = 0;

        for (let index = 0; index < timedSections.length; index += 1) {
          const duration =
            Number(timedSections[index]?.durationMinutes || 0) * 60;
        ...
        if (activeIndex !== activeSectionIndex) {
          ...
          const nextQuestions = questions.filter(
            (question) =>
              Number(question.sectionId) ===
              Number(timedSections[activeIndex]?.id)
          );

          if (nextQuestions[0]) {
            setCurrentQuestionId(Number(nextQuestions[0].id));
          }
        }
        ...
        if (
          remaining === 0 &&
          nextActiveSeconds > 0 &&
          activeIndex === timedSections.length - 1 &&
          !expirySubmitted
        ) {
          expirySubmitted = true;
          submitTestRef.current?.(true);
       ...
       * -------------------------------------------------------------
       */
      const total = Math.max(0, Number(test?.durationMinutes || 0) * 60);
      const remaining = Math.max(0, total - nextActiveSeconds);

      setRemainingSeconds(remaining);

      if (total > 0 && remaining === 0 && !expirySubmitted) {
        expirySubmitted = true;
        submitTestRef.current?.(true);
      }
    };
  ...
  */
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (document.visibilityState !== "visible" || !timerRunningRef.current) {
        return;
      }

      setQuestionsState((prev) => {
        if (!currentQuestionId || !prev[currentQuestionId]) {
          return prev;
        }
        ...
        const next = {
          ...
          [currentQuestionId]: {
            ...
            timeSpentSeconds:
              Number(prev[currentQuestionId].timeSpentSeconds || 0) + 1,
          },
        };
    ...
    return () => window.clearInterval(timer);
  }, [attempt?.id, loading, currentQuestionId]);
  ...
  */
  useEffect(() => {
    if (loading || !attempt?.id || submittedRef.current) {
      return;
    }
  ...
  }, [loading, attempt?.id, enterFullscreen]);
  ...
  */
  const registerViolation = useCallback((reason) => {
    if (submittedRef.current || !attemptRef.current?.id) {
      return;
    }
    ...
    const next = Math.min(3, fullscreenWarningsRef.current);

    setFullscreenWarnings(next);
    ...
    if (next >= 3) {
      if (forceSubmitTimerRef.current) {
        clearTimeout(forceSubmitTimerRef.current);
      }

      forceSubmitTimerRef.current = window.setTimeout(() => {
        setShowFullscreenWarning(false);
        submitTestRef.current?.(true);
      }, 800);
    }
  ...
  */
  ...
  }, [loading, attempt?.id, registerViolation]);
  ...
  *
  * IMPORTANT:
  * The original warning behavior is preserved.
  *
  * Only self-paced attempts pause their timer.
  * Live attempts continue against their fixed deadline.
  */
  useEffect(() => {
    if (loading || !attempt?.id) {
      return;
    ...
    const onVisibilityChange = async () => {
      if (document.visibilityState === "hidden") {
        ...
        /*
         * Pause only self-paced tests.
         */
        if (timerMode === "active") {
          timerRunningRef.current = false;

          void sendTimerAction("pause", { keepalive: true });
        }

        /*
         * Preserve existing answer checkpoint.
         */
        void saveCheckpoint({ keepalive: true });
      ...
      if (document.visibilityState === "visible" && tabWasHiddenRef.current) {
        ...
        /*
         * Resume only self-paced tests.
         */
        if (timerMode === "active" && !submittedRef.current) {
          await sendTimerAction("resume");
        }

        /*
         * Preserve original violation warning.
         */
  ...
  }, [attempt?.id, loading, registerViolation, saveCheckpoint, sendTimerAction]);
  ...
  */
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if (blocked) {
        ...
        toast.warning("This action is disabled during the test.");
      }
    };
  ...
  /*
  |--------------------------------------------------------------------------
  | PAGE CLOSE / BACK / RELOAD
  |--------------------------------------------------------------------------
  *
  * For self-paced tests, persist the paused state before
  * the document disappears.
  *
  * LIVE tests don't need a pause request.
  */
  useEffect(() => {
    if (!attempt?.id || loading) {
      return;
  ...
  }, [attempt?.id, id, loading, saveCheckpoint, sendTimerAction, series]);
  ...
  */
  useEffect(() => {
    ...
    const handleKey = (event) => {
      ...
      if (event.key === "ArrowLeft") {
        event.preventDefault();
        goToQuestion(sectionQuestions[currentSectionQuestionIndex - 1]?.id);
      }
    };
  ...
  }, [goToQuestion, sectionQuestions, currentSectionQuestionIndex, loading]);
  ...
  const selectOption = (optionIndex) => {
    ...
    setQuestionsState((prev) => ({
      ...
      [currentQuestion.id]: {
        ...
        visited: true,
        selectedOption:
          prev[currentQuestion.id]?.selectedOption === optionIndex
            ? null
            : optionIndex,
      },
    }));
  ...
  */
  const nextQuestion = useCallback(() => {
    const index = sectionQuestions.findIndex(
      (question) => Number(question.id) === Number(currentQuestionId)
    );
     ...
     */
    const next = sectionQuestions[index + 1];
    ...
    /*
     * Never manually open the next section.
     * Timer controls section unlock.
     */
    if (sectional && activeSectionIndex < timedSections.length - 1) {
      return;
    }
  }, [
    sectionQuestions,
  ...
  const previousQuestion = useCallback(() => {
    ...
    const previous = sectionQuestions[index - 1];

    if (previous) {
      goToQuestion(previous.id);
    }
  }, [sectionQuestions, currentQuestionId, goToQuestion]);
  ...
  */
  const selectSection = (index) => {
    if (!sectional || !timedSections[index]) {
      return;
    }

    if (timedSections[index].isSequential && index > activeSectionIndex) {
      return;
    }
    ...
    const first = questions.find(
      (question) =>
        Number(question.sectionId) === Number(timedSections[index].id)
    );
  ...
  */
  const currentState = currentQuestion
    ? questionsState[currentQuestion.id] || {
        visited: false,
        selectedOption: null,
        markedForReview: false,
        timeSpentSeconds: 0,
  ...
  |--------------------------------------------------------------------------
  */
  const currentTimerMode = String(
    attempt?.timerMode || (attempt?.eventId ? "live" : "active")
  ).toLowerCase();

  const headerTime = isDpp
    ? elapsedSeconds
    : sectional || fixedTimer || currentTimerMode === "active"
    ? remainingSeconds
    : elapsedSeconds;
  ...
  */
  if (loading) {
    return <SpiderManLoader text="Loading Test..." />;
  }
  ...
  |--------------------------------------------------------------------------
  */
  if (loadError || !test || !attempt || !currentQuestion) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-5">
        <div className="w-full max-w-[500px] rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">
            {loadError || "Something went wrong while loading this test."}
          </p>
    ...
    <div className="fixed inset-0 z-[100] flex h-screen h-[100dvh] w-screen flex-col overflow-hidden bg-[#f8fafc] font-sans">
        ...
        <div className="flex h-[72px] items-center justify-between px-4 sm:px-5 lg:px-8">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 truncate text-xs font-medium text-slate-500">
              {test.seriesName} · Attempt {attempt.attemptNumber}
            </div>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <div
              className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-extrabold ${
                headerTime <= 60 && (sectional || fixedTimer)
        ...
        {sectional && (
          <div className="flex min-h-[58px] items-center gap-2 overflow-x-auto border-t border-slate-200 bg-[#f1f5f9] px-3 sm:px-5 lg:px-8">
            {timedSections.map((section, index) => {
              ...
              const locked =
                Boolean(section.isSequential) && index > activeSectionIndex;
      ...
      <div className="flex min-h-0 flex-1">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-6 pb-28 lg:px-8 lg:pb-8">
              <div className="mx-auto w-full max-w-[1000px]">
                ...
                {currentQuestion.questionImageUrl ? (
                  <div
                    className={`${
                      currentQuestion.questionText ? "mt-7" : ""
                ...
                <div className="relative left-1/2 mt-7 w-[calc(100%+16px)] -translate-x-1/2 space-y-3 sm:w-[calc(100%+24px)]">
                  {currentQuestion.options.map((option, index) => {
                    const selected = currentState?.selectedOption === index;
              ...
              <div className="mx-auto flex w-full max-w-[1000px] items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={!sectionQuestions[currentSectionQuestionIndex - 1]}
                  ...
                  {isLastQuestionOfEntireTest ? (
                    ...
                    <button
                      ...
                      disabled={sectionBoundaryLocked}
        ...
        {desktopPalette && (
          <aside className="hidden w-[340px] shrink-0 border-l border-slate-200 bg-[#f8fafc] lg:flex lg:flex-col">
            <div className="flex h-[72px] shrink-0 items-center justify-between border-b border-slate-200 bg-white px-5">
              ...
              <button
                type="button"
                onClick={() => setDesktopPalette(false)}
            ...
            <div className="grid shrink-0 grid-cols-2 gap-3 border-b border-slate-200 bg-white p-5">
              <div className="flex items-center gap-2">
                <LegendIcon type={STATUS.ANSWERED} count={answeredCount} />
              ...
              <div className="flex items-center gap-2">
                <LegendIcon type={STATUS.NOT_VISITED} count={notVisitedCount} />
                <span className="text-xs font-semibold text-slate-600">
                  Not Visited
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <LegendIcon type={STATUS.REVIEW} count={reviewOnlyCount} />
                <span className="text-xs font-semibold text-slate-600">
                  Review
                </span>
              </div>
                  ...
                  return (
                    ...
                    >
                      <PaletteIcon
                        ...
                        current={
                          Number(currentQuestion.id) === Number(question.id)
                        }
                      />
                    </button>
                  );
        ...
        {!desktopPalette && (
          <button
            type="button"
            onClick={() => setDesktopPalette(true)}
        ...
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-slate-950/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-[430px] rounded-3xl bg-white p-7 text-center shadow-2xl">
            ...
            <h3 className="mt-5 text-xl font-black text-slate-900">
              {fullscreenWarnings >= 3 ? "Final Warning" : "Fullscreen exited"}
            </h3>
            ...
            <div className="mt-5 rounded-xl bg-slate-50 px-4 py-3 text-sm font-bold text-slate-600">
              Warning {Math.min(fullscreenWarnings, 3)} / 3
            </div>
